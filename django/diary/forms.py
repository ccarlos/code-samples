...
from diary.models import Comment, DiaryEntry
...
ENTRY_EMPTY = _lazy(u'Write your entry here...')
SET_DRAFT = _lazy(u'Cannot set a published entry to a draft.')


class DiaryForm(forms.ModelForm):
    """A diary entry form that requires min characters if not a draft."""
    is_draft = forms.BooleanField(required=False,
                                  error_messages={'invalid': SET_DRAFT})
    text = StrippedCharField(
    ...
    def save(self, commit=True, *args, **kwargs):
        ...
        if commit:
            ...
            diary.updated = datetime.datetime.now()
            diary.save()
        return diary

...
# $Id$
#
# Print a page of postscript showing color codes.
#
...
sub load_colors () {
   ...
   while (defined (my $line = <DATA>)) {
      next unless $line =~ m/^(\d+)\s+(\d+)\s+(\d+)\s+(.*)/;
...
sub showcolor ($$$) {
   my ($xpoints, $ypoints, $state) = @_;
}

sub main () {
   my $state = {INFILENAME=> "-", INFILE=> *STDIN, OUTFILE=> *STDOUT};
...
__DATA__
! The following data was borrowed from /usr/X11/lib/X11/rgb.txt
! A bang, rather than a hash is used as a comment marker here.
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
